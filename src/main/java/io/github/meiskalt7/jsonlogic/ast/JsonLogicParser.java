...
public final class JsonLogicParser {
    ...
    private static JsonLogicNode parse(JsonElement root) throws JsonLogicParseException {
        ...
        if (object.keySet().isEmpty()) {
            return new JsonLogicVariable(JsonLogicNull.NULL, JsonLogicNull.NULL);
        }
        ...
        if ("var".equals(key)) {
            JsonLogicNode defaultValue =
                arguments.size() > 1 ? arguments.get(1) : JsonLogicNull.NULL;
            return new JsonLogicVariable(
                arguments.size() < 1 ? JsonLogicNull.NULL : arguments.get(0), defaultValue);
        }
